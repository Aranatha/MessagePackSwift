...
func testPackMap(_ count: Int, prefix: Data) {
    ...
    XCTAssertEqual(packed.subdata(in: 0 ..< prefix.count),
                   prefix)

    var remainder = Subdata(data: packed,
                            startIndex: prefix.count,
                            endIndex: packed.count)
    var keys = Set<Int>()
    do {
        for _ in 0 ..< count {
            let entry = try unpack(remainder)
            let key = Int(try entry.value.int64Value())

            XCTAssertFalse(keys.contains(key))
            keys.insert(key)

            let nilValue = try unpack(entry.remainder)
            XCTAssertEqual(nilValue.value,
                           MessagePackValue.nil)

            remainder = nilValue.remainder
        }
    } catch {
        print(error)
        XCTFail()
...
class MapTests: XCTestCase {
    ...
    func testUnpackFixmap() {
        ...
        XCTAssertEqual(unpacked?.value,
                       MessagePackValue.map([.string("c"): .string("cookie")]))
        XCTAssertEqual(unpacked?.remainder.count,
                       0)
    }

    func testPackMap16() {
        testPackMap(16,
                    prefix: Data([0xde, 0x00, 0x10]))
    }

    func testUnpackMap16() {
        let unpacked = try? unpack(Data([0xde, 0x00, 0x10]) + payload(16))
        XCTAssertEqual(unpacked?.value,
                       MessagePackValue.map(map(16)))
        XCTAssertEqual(unpacked?.remainder.count,
                       0)
    }
}
